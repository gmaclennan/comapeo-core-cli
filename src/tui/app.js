...
export async function startTui({ storage, io, session: injectedSession }) {
  ...
  const hyperlinksEnabled = supportsHyperlinks(stdout)
  ...
  const state = {
    ...
    record: undefined,
    /** Resolved attachment media URLs for the open record (name → url). @type {Map<string, string> | undefined} */
  ...
  function showRecordList(group) {
    showList({
      ...
      onSelect: (d) => {
        state.record = d
        state.recordLinks = undefined
        state.recordBack = () => showRecordList(group)
        state.screen = 'record'
        repaint()
        // Resolve attachment media URLs in the background, then repaint the
        // record so they render (as clickable links where supported).
        void resolveRecordLinks(d)
      },
  ...
  /**
   * Resolve media-server URLs for a record's attachments and stash them on
   * state, repainting if the same record is still open. Best-effort: a failed
   * lookup just leaves that attachment without a URL.
   * @param {Record<string, any>} doc
   */
  async function resolveRecordLinks(doc) {
    if (!state.projectId || !Array.isArray(doc.attachments)) return
    ...
    for (const att of doc.attachments) {
      try {
        // 'original' is the only variant guaranteed to exist on this archive
        // device (previews are generated by the authoring device); opening the
        // full file in a browser is what we want anyway.
        const url = await project.$blobs.getUrl({
          type: att.type,
          variant: 'original',
          name: att.name,
          driveId: att.driveDiscoveryId,
        })
        links.set(att.name, url)
      } catch {
        // unsupported type or no media base URL — skip
      }
    }
    if (state.screen === 'record' && state.record === doc) {
      state.recordLinks = links
      repaint()
    }
  }
